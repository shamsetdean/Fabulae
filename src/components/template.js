              ...
              <template x-for="show in searchResults" :key="show.id">
                <div class="card p-2.5 flex items-center gap-3">
                  <template x-if="show.poster">
                    <img :src="show.poster" :alt="show.name" class="w-9 h-12 rounded object-cover flex-shrink-0" loading="lazy" decoding="async" />
                  </template>
                  <template x-if="!show.poster">
              ...
              <template x-for="item in filtered" :key="item.id">
                <li class="card p-2.5 flex gap-2.5">
                  <a :href="'#/show/' + item.show.id" class="flex-shrink-0">
                    <img :src="item.show.poster" :alt="item.show.name" loading="lazy" decoding="async" class="w-12 h-18 rounded-md bg-ink-800 object-cover" />
                  </a>
                  <div class="flex-1 min-w-0">
                ...
                <div class="space-y-2">
                  <template x-for="show in searchResults" :key="show.id">
                    <div class="card p-3 flex gap-3 items-center">
                      <template x-if="show.poster">
                        <img :src="show.poster" :alt="show.name" class="w-12 h-16 rounded-lg object-cover flex-shrink-0" loading="lazy" decoding="async" />
                      </template>
                      <template x-if="!show.poster">
                ...
                <div class="card p-3 flex gap-3 items-center">
                  <a :href="'#/show/' + entry.tmdb_id" class="flex-shrink-0">
                    <template x-if="entry.show?.poster">
                      <img :src="entry.show.poster" :alt="entry.show.name" class="w-14 h-20 rounded-lg object-cover" loading="lazy" decoding="async" />
                    </template>
                    <template x-if="!entry.show?.poster">
                          ...
                          <template x-for="(show, idx) in currentTop.shows" :key="'t-' + show.id">
                            <a :href="'#/show/' + show.id" class="flex-shrink-0 w-24 snap-start group">
                              <div class="poster-compact relative">
                                <template x-if="show.poster">
                                  <img :src="show.poster" :alt="show.name" loading="lazy" decoding="async" class="group-hover:scale-105 transition-transform duration-500" />
                                </template>
                                <template x-if="!show.poster">
                        ...
                        <div class="flex gap-2 overflow-x-auto -mx-1 px-1 pb-2 snap-x">
                          <template x-for="(show, idx) in currentFlop.shows" :key="'f-' + show.id">
                            <a :href="'#/show/' + show.id" class="flex-shrink-0 w-24 snap-start group">
                              <div class="poster-compact relative">
                                <template x-if="show.poster">
                                  <img :src="show.poster" :alt="show.name" loading="lazy" decoding="async" class="grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-500" />
                                </template>
                                <template x-if="!show.poster">
                                  <div class="aspect-[2/3] bg-ink-800 rounded"></div>
                                </template>
                            ...
                            <a :href="'#/show/' + item.tmdb_id" class="block group">
                              <div class="aspect-[2/3] rounded-lg overflow-hidden bg-ink-800 relative">
                                <template x-if="item.show?.poster">
                                  <img :src="item.show.poster" :alt="item.show.name" loading="lazy" decoding="async" class="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                                </template>
                                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>
                                <template x-if="item.recommendation === 'recommended'">
                                ...
                                <!-- Badge "En commun" (si je suis ce profil et j'ai aussi cette série) -->
                                <template x-if="hasInCommon(item.tmdb_id)">
                                  <span class="absolute bottom-1.5 left-1.5 inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full bg-flame-600/90 text-cream-50 font-medium backdrop-blur-sm">
